...
for i,(train_index, test_index) in enumerate(kf.split(dfX, y)):
    ...
    score_rf_crps = crps(y_test, model.predict(X_test))

    print(score_rf_crps)

    score.append(score_rf_crps)

    models.append(model)

print(np.mean(score_rf_crps))
# Submit results
...
for (test_df, sample_prediction_df) in env.iter_test():     
    ...
    y_pred = np.mean([model.predict(dfb_sc_final) for model in models],axis=0)
