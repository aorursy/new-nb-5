...
coordinates = pd.read_csv("../input/countrydatafile/coordinates.csv")

global_data = pd.read_csv("../input/countrydatafile/Global_Data_by_Country_2019.csv")
coordinates.head()
...
#global_data=global_data.drop(columns=["Population"])
train.head()
...
train=train.drop(columns=["Province/State"], axis=1)
...
X_test=X_test.drop(columns=["Province/State"], axis=1)
del global_data
...
X_test=X_test.drop(["Province", "ForecastId"], axis=1)
