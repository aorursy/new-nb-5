...
recommender =  turicreate.recommender.item_similarity_recommender.create(train_data, user_id='user_id', item_id='product_id', target='target', similarity_type='pearson', verbose=True)
...
for key,item in thresholds.items():
    ...
    results[key].to_csv('submission_turi_ItemSim_pearson_pol'+str(key)+'.csv',index=False)