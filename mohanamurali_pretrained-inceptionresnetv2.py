...
early_stop = EarlyStopping(monitor='val_loss', min_delta=0.0001, patience=2, verbose=1)
...
for i in range(len(model.layers)):
    ...
    model.layers[i].trainable = False # On vérouille toutes les couches

for i in range(len(model.layers)-10,len(model.layers)):

    model.layers[i].trainable = True #et on dévérouille les 10 dernières
...
# according to https://papers.nips.cc/paper/7003-the-marginal-value-of-adaptive-gradient-methods-in-machine-learning.pdf sgd generalize better than adam, but slower
