...
################
df_res[['image_name', 'pred']].groupby('image_name').mean().reset_index().rename({'pred': 'target'}, axis=1).to_csv("submission_b3.csv", index=False)
VERBOSE = 1#
PLOT    = 1
EFF_NET = 4
...
################
df_res[['image_name', 'pred']].groupby('image_name').mean().reset_index().rename({'pred': 'target'}, axis=1).to_csv("submission_b4.csv", index=False)
#xxx = df_oof.groupby('image_name').mean().reset_index().merge(df_base_train, on='image_name')
#print("OOF AUC (TTA %d) = %.4f" % (TTA, metrics.roc_auc_score(xxx.target, xxx.pred)))
