...
X = pd.concat([pd.concat([dchunk, nchunk], axis=1).sample(frac=0.10)

               for dchunk, nchunk in zip(date_chunks, num_chunks)])

y = pd.read_csv("../input/train_numeric.csv", index_col=0, usecols=[0,969], dtype=np.float32).loc[X.index].fillna(-3).values.ravel()
...
X = np.concatenate([
    ...
    pd.read_csv("../input/train_date.csv", index_col=0, dtype=np.float32,
                ...
                usecols=np.concatenate([[0], important_indices[important_indices < n_date_features] + 1])).fillna(-3).values,

    pd.read_csv("../input/train_numeric.csv", index_col=0, dtype=np.float32,

                usecols=np.concatenate([[0], important_indices[important_indices >= n_date_features] + 1 - 1156])).fillna(-3).values

], axis=1)

y = pd.read_csv("../input/train_numeric.csv", index_col=0, dtype=np.float32, usecols=[0,969]).fillna(-3).values.ravel()
clf = XGBClassifier(max_depth=10, learning_rate=0.07, n_estimators=150,scale_pos_weight=0.9)
