...
grpSolo = grp[cols].sum().describe().T[aggs]
# summary of team-match
grp = train.loc[~train['matchType'].str.contains('solo')].groupby('matchId')
grpTeam = grp[cols].sum().describe().T[aggs]

pd.concat([grpSolo, grpTeam], keys=['solo', 'team'], axis=1)
cols.extend(['killPlace','winPlacePerc'])
grpMean = train.groupby('matchId')[cols].mean()
grpMax = train.groupby('matchId')[cols].max()
fig, ax = plt.subplots(2, 1, figsize=(12, 12), sharey=True)
for df, ax in zip([grpMean,grpMax], ax.ravel()):
    sns.heatmap(df.corr(), annot=True, linewidths=.6, fmt='.2f', 
                vmax=1, vmin=-1, center=0, cmap='Blues', ax=ax)
...
def grouping(df):
    ...
    gCount = group.size().to_frame('member')
    gMean = group.mean()
...
for c in gcol:
    all_data['_diff1.' + c] = all_data['_max.' + c] - all_data['_min.' + c]
    all_data['_diff2.' + c] = all_data['_max.' + c] - all_data[c]
print(all_data.shape)
...
for c in agg_col:
    all_data['_perc.sum.' + c] = all_data[c] / all_data['_msum.' + c]
    
fillInf(all_data, 0)
''' percentage of match mean
'''
matchMean = match.mean().rename(columns=lambda s: '_mmean.' + s).reset_index()
...
efcols = ['kills','killPlace','_killPlaceOverMaxPlace','damageDealt','walkDistance','revives']
for c in agg_col:
    all_data['_perc.mean.' + c] = all_data[c] / all_data['_mmean.' + c]
    if c not in efcols:
        all_data.drop(['_mmean.' + c], axis=1, inplace=True)
