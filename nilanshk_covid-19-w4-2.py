...
#cc_dates=train_data.groupby('Date').sum()['ConfirmedCases']

#print(cc_dates)

#cc_dates=train_data.groupby('Date').sum()['ConfirmedCases'].reset_index()
...
#test_data[features] = scaler.fit_transform(test_data[features].values)
...
df['ConfirmedCases']=np.round(df['ConfirmedCases'])

df['Fatalities']=np.round(df['Fatalities'])
