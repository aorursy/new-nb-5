...
total = preprocess(total, country_info = country_info, longlat = longlat, lags = False)
...
X_test, c_test = preprocess_test_RFC(test, X_train, regressors, dependant)
X_train = pd.concat([X_train.loc[(c_train.loc[c_train != 0]).index], X_train.loc[(c_train.loc[c_train == 0]).index].sample(frac = 0.1)])

c_train = c_train.loc[X_train.index]
X_train.shape
...
RMSE_region.sort_values(ascending = True).tail(10).plot.bar()
...
rfc_fatalities = GradientBoostingRegressor(loss = 'huber',
...
print_feature_importances(rfc_fatalities, X_train.columns)
