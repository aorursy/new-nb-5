...
for n, id_ in tqdm_notebook(enumerate(train_ids), total=len(train_ids)):
    ...
    x_img = resize(x_img, (200, 200, 1), mode='constant', preserve_range=True)
...
f_repeat = RepeatVector(16*16)(input_features)
f_conv = Reshape((16, 16, n_features))(f_repeat)
...
results = model.fit({'img': X_train, 'feat': X_feat_train}, y_train, batch_size=16, epochs=20, callbacks=callbacks,
...
for n, id_ in tqdm_notebook(enumerate(test_ids), total=len(test_ids)):
    ...
    x = resize(x, (200, 200, 1), mode='constant', preserve_range=True)
