...
#change y_preds from multi-label to single label

idx = np.argmax(y_preds, axis=-1)

y_preds_new = np.zeros((y_preds.shape[0],1))

for i in range(len(idx)):

    if idx[i]==0:y_preds_new[i]=1
...
accuracy_score(y_preds_new, y_new)
f1_score(y_new, y_preds_new)