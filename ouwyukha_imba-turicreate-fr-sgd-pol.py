...
recommender =  turicreate.recommender.factorization_recommender.create(train_data, user_id='user_id', item_id='product_id', target='target', solver='sgd', verbose=True)
...
for key,item in thresholds.items():
    ...
    results[key].to_csv('submission_turi_fr_sgd_pol_'+str(key)+'.csv',index=False)