...
PLOT    = 1
EFF_NET = 5
model_name_dir='256-2-effi5-imagenet'
...
################
df_res.to_csv('../working/test_res_all_b5.csv', index=False)
df_oof.to_csv('../working/oof_res_all_b5.csv', index=False)
################
df_res[['image_name', 'pred']].groupby('image_name').mean().reset_index().rename({'pred': 'target'}, axis=1).to_csv("submission_b5.csv", index=False)
VERBOSE = 1#
PLOT    = 1
EFF_NET = 6
model_name_dir='256-2-effi6-imagenet'
...
print("OOF AUC (TTA %d) = %.4f" % (TTA, metrics.roc_auc_score(xxx.target, xxx.pred)))
################
df_res.to_csv('../working/test_res_all_b6.csv', index=False)
df_oof.to_csv('../working/oof_res_all_b6.csv', index=False)
################
df_res[['image_name', 'pred']].groupby('image_name').mean().reset_index().rename({'pred': 'target'}, axis=1).to_csv("submission_b6.csv", index=False)
#xxx = df_oof.groupby('image_name').mean().reset_index().merge(df_base_train, on='image_name')
#print("OOF AUC (TTA %d) = %.4f" % (TTA, metrics.roc_auc_score(xxx.target, xxx.pred)))
#df_res.to_csv('../working/test_res_all.csv', index=False)
#df_oof.to_csv('../working/oof_res_all.csv', index=False)
df_res[['image_name', 'pred']].groupby('image_name').mean().reset_index().rename({'pred': 'target'}, axis=1).to_csv("submission_6_20.csv", index=False)
