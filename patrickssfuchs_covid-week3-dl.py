...
#from func import create_dataset, load_data, create_dataset_daily
...
def create_dataset(seq_len, scaler = None):
    ...
    #test = test[test["Date"] > date] #MUDAR DE ACORDO COM O DATASET
...
for i in [x_train_window,x_train_place,y_train,x_val_window,x_val_place,y_val]:

    print(i.shape)
for i in [x_train_window,x_train_place,y_train]:
...
lstm = RNN(LMUCell(units=212, order = 256, theta = 212), return_sequences=False)(inp_window)
...
cp = ModelCheckpoint("test_sub.hdf5", monitor='loss', verbose=1, save_best_only=True, save_weights_only= True)
...
plt.title("Expected Public Loss: {}".format(np.mean(list(paises.values()))))
...
#df_predict[df_predict["Country_Region"] == 'Italy']
days = 75

date = str(pd.to_datetime(pd.to_datetime(date_train).value - (60*60*24)*10**9*days))[:-9]
...
for i in tqdm(range(len(df_predict)), desc = "predict"):
    ...
    if df_predict.loc[i, "Date"] < date_train:
        ...
        if seq_len < max_look_back:
           ...
           #tmp[0][-seq_len:] = df[["ConfirmedCases", "Fatalities"]].iloc[i-seq_len:i]
        ...
        #if df_predict.loc[i, "Date"] > "2020-04-06":

        #    df_predict.loc[i,"ConfirmedCases"], df_predict.loc[i,"Fatalities"] = 0.0, 0.0
...
#predict_data = df_predict[df_predict["ForecastId"] != 0]
...
for i in tqdm(range(y_true.shape[0])):
    ...
    if y_true["Date"].iloc[i] > "2020-04-06":
        ...
        y_true["ConfirmedCases"].iloc[i] = y_true["ConfirmedCases"].iloc[i-1]

        y_true["Fatalities"].iloc[i] = y_true["Fatalities"].iloc[i-1]
...
y_pred = y_pred[["ForecastId", "ConfirmedCases", "Fatalities"]]
