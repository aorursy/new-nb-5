...
for file in os.listdir("../input/word2vec-nlp-tutorial"):

    print(file)
#Matplot lib 시각화 파라미터 셋
...
additional_data = pd.read_csv("../input/masterdataset/master_dataset.csv")

additional_data.columns
additional_data = additional_data.drop(['Unnamed: 0'], axis=1)
...
# IMDB 데이터셋을 임포팅하고 이게 모델 정확도에 큰 기여를 함.
...
additional_data = additional_data[usecols]
...
len(Y_sub_pred)
Y_sub_pred = np.append(Y_sub_pred, [0,1])
