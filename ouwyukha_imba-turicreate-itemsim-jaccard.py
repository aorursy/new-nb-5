...
recommender =  turicreate.recommender.item_similarity_recommender.create(train_data, user_id='user_id', item_id='product_id', target='target', item_data=content_data, similarity_type='jaccard', verbose=True)
...
for key,item in thresholds.items():
    ...
    results[key].to_csv('submission_turi_ItemSim_jaccard_'+str(key)+'.csv',index=False)