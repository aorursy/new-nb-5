...
def additional_data():
    ...
    df_add_data = pd.read_csv('../input/btcusd01/btcusd.csv')

    df_add_data['ds'] = pd.to_datetime(df_add_data['ds'])

    df_add_data['y2'] = (df_add_data['y'] - df_add_data['y'].shift(1) )/df_add_data['y'].shift(1)

    df_add_data['y2'].fillna(0.0,inplace=True)

    df_add_data['5MA'] = df_add_data['y'].rolling(5).mean()

    df_add_data['5MA_diff'] = (df_add_data['y'] - df_add_data['5MA']) / df_add_data['5MA']

    df_add_data['5std'] = df_add_data['y'].rolling(5).std()

    df_add_data['5BBD'] = ((df_add_data['y'] - df_add_data['5MA']) / df_add_data['5std']).round(2)

    df_add_data['15MA'] = df_add_data['y'].rolling(15).mean()

    df_add_data['15MA_diff'] = (df_add_data['y'] - df_add_data['15MA']) / df_add_data['15MA']

    df_add_data['15std'] = df_add_data['y'].rolling(15).std()

    df_add_data['15BBD'] = ((df_add_data['y'] - df_add_data['15MA']) / df_add_data['15std']).round(2)

    df_add_data['30MA'] = df_add_data['y'].rolling(30).mean()

    df_add_data['30MA_diff'] = (df_add_data['y'] - df_add_data['30MA']) / df_add_data['30MA']

    df_add_data['30std'] = df_add_data['y'].rolling(30).std()

    df_add_data['30BBD'] = ((df_add_data['y'] - df_add_data['30MA']) / df_add_data['30std']).round(2)

    df_add_data['60MA'] = df_add_data['y'].rolling(60).mean()

    df_add_data['60MA_diff'] = (df_add_data['y'] - df_add_data['60MA']) / df_add_data['60MA']

    df_add_data['60std'] = df_add_data['y'].rolling(60).std()

    df_add_data['60BBD'] = ((df_add_data['y'] - df_add_data['60MA']) / df_add_data['60std']).round(2)
...
#★★★マーチャントデータのデータフレームを返す

def merchant_data():

    df_merchant = pd.read_csv('../input/elo-merchant-category-recommendation/merchants.csv')

    df_merchant['category_1'] = df_merchant['category_1'].map({'Y': 1, 'N': 0}).astype(int)

    df_merchant['category_4'] = df_merchant['category_4'].map({'Y': 1, 'N': 0}).astype(int)

    df_merchant['most_recent_sales_range'] = df_merchant['most_recent_sales_range'].map({'A':4, 'B':3, 'C':2, 'D':1, 'E':0}).astype(int)

    df_merchant['most_recent_purchases_range'] = df_merchant['most_recent_purchases_range'].map({'A':4, 'B':3, 'C':2, 'D':1, 'E':0}).astype(int)

    df_merchant['avg_sales_lag3'] = df_merchant['avg_sales_lag3'].fillna(0)

    df_merchant['avg_sales_lag6'] = df_merchant['avg_sales_lag6'].fillna(0)

    df_merchant['avg_sales_lag12'] = df_merchant['avg_sales_lag12'].fillna(0)

    df_merchant['category_2'] = df_merchant['category_2'].fillna(-1)

    

    df_merchant.columns = [('field_'+ c) if c != 'merchant_id' else 'merchant_id' for c in df_merchant.columns ]#結合用にIDだけはそのままで、他は識別子を付ける
...
def train_test(num_rows=None):
    ...
    train_df = pd.read_csv('../input/elo-merchant-category-recommendation/train.csv', index_col=['card_id'], nrows=num_rows)

    test_df = pd.read_csv('../input/elo-merchant-category-recommendation/test.csv', index_col=['card_id'], nrows=num_rows)
    ...
    df_add_data = additional_data()#★★★ビットコインのデータフレームを用意

    df_add_data = df_add_data.rename(columns={'ds': 'first_active_month'})#★★★下記の結合のため、カラムの名前を変更

    df = df.reset_index().merge( df_add_data, on='first_active_month', how='left').set_index('card_id')#★★★ビットコインのデータを訓練データに結合

    del df_add_data;gc.collect()#★★★メモリ節約のため、手動でガベージコレクション。
...
def historical_transactions(num_rows=None):
    ...
    hist_df['ds'] = hist_df['purchase_date'].dt.date#★★★参考にしたカーネルでは日付のカラムが無かったため、結合用に追加    
    ...
    df_add_data['ds'] = df_add_data['ds'].dt.date#★★★下記の結合のため、'ds'カラムを用意

    hist_df = pd.merge(hist_df, df_add_data, on='ds', how='left')#★★★ビットコインのデータをhistデータに結合
    ...
    aggs['y'] = ['sum','max','min','mean','var']#★★★ビットコインの終値価格
...
def new_merchant_transactions(num_rows=None):
    ...
    new_merchant_df = pd.read_csv('../input/elo-merchant-category-recommendation/new_merchant_transactions.csv', nrows=num_rows)
    ...
    new_merchant_df['ds'] = new_merchant_df['purchase_date'].dt.date#★★★参考にしたカーネルでは日付のカラムが無かったため、結合用に追加      
    ...
    new_merchant_df = pd.merge(new_merchant_df, df_add_data, on='ds', how='left')#★★★ビットコインのデータをnew_merchantデータに結合

    del df_add_data;gc.collect()#★★★メモリ節約のため、手動でガベージコレクション。

    

    merchant_df = merchant_data()#★★★マーチャントデータのデータフレームを用意

    new_merchant_df = pd.merge(new_merchant_df,merchant_df, on='merchant_id', how='left')#★★★マーチャントデータをnew_merchantデータに結合

    del merchant_df;gc.collect()#★★★メモリ節約のため、手動でガベージコレクション。
    ...
    col_unique =['subsector_id', 'merchant_id', 'merchant_category_id','field_merchant_group_id','field_city_id']
    ...
    aggs['field_category_1'] = ['mean']

    aggs['field_category_2'] = ['mean']

    aggs['field_category_4'] = ['mean']

    aggs['field_numerical_1'] = ['mean']

    aggs['field_numerical_2'] = ['mean']

    aggs['field_most_recent_sales_range'] = ['mean','max','min']

    aggs['field_most_recent_purchases_range'] = ['mean','max','min']

    aggs['field_avg_sales_lag3'] = ['mean','max','min']

    aggs['field_avg_purchases_lag3'] = ['mean','max','min']

    aggs['field_active_months_lag3'] = ['mean','max','min']

    aggs['field_avg_sales_lag6'] = ['mean','max','min']

    aggs['field_avg_purchases_lag6'] = ['mean','max','min']

    aggs['field_active_months_lag6'] = ['mean','max','min']

    aggs['field_avg_sales_lag12'] = ['mean','max','min']

    aggs['field_avg_purchases_lag12'] = ['mean','max','min']

    aggs['field_active_months_lag12'] = ['mean','max','min']
...
def kfold_lightgbm(train_df, test_df, num_folds, stratified = False, debug= False,FileName = 'submission'):
    ...
    if not debug:
        ...
        test_df[['card_id', 'target']].to_csv(FileName, index=False)

    

    return feature_importance_df
#以下、実処理
num_rows = None#10000# if debug else None

df = train_test(num_rows)

df.head()
hist = historical_transactions(num_rows)

hist.head()
df = pd.merge(df, hist, on='card_id', how='outer')
merch = new_merchant_transactions(num_rows)

merch.head()
df = pd.merge(df, merch , on='card_id', how='outer')
...
best_features = kfold_lightgbm(train_df, test_df, num_folds=11, stratified=False, debug=debug,FileName='submission_1')
#★★★重要度の低い特徴量を抽出

best_data = best_features.sort_values(by="importance",ascending=False)

remove_label = best_data.tail(500)

remove_label.tail(10)



remove_columns = [c for c in remove_label["feature"] ]
...
train_df_new = train_df.drop(remove_columns,axis=1)

test_df_new = test_df.drop(remove_columns,axis=1)
#★★★改めて再検証

kfold_lightgbm(train_df_new, test_df_new, num_folds=11, stratified=False, debug=debug,FileName='submission_2')